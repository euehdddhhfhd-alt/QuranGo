...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderVersesFromSurahNum(num){
    ...
    const data = (window.AyahHubData && window.AyahHubData[key]) || null;
  ...
  // Temporary AdSense debug helper — logs status of ad slots and highlights missing ones
  // Remove this helper after verifying ads
  setTimeout(() => {
    try {
      const ads = document.querySelectorAll('ins.adsbygoogle');
      if (ads.length === 0) {
        console.warn('AdSense debug: no `ins.adsbygoogle` elements found on the page.');
      } else {
        ads.forEach((ad, i) => {
          const hasContent = ad.childElementCount > 0 || (ad.innerHTML && ad.innerHTML.trim().length > 0);
          if (!hasContent) {
            console.warn(`AdSense debug: ad #${i+1} appears empty or not filled yet.`, ad);
            ad.classList.add('ads-debug-missing');
            ad.style.outline = '2px dashed #f00';
            ad.title = 'AdSense not filled: check console/network and AdSense account';
          } else {
            console.info(`AdSense debug: ad #${i+1} has content.`, ad);
          }
        });
      }
    } catch (e) {
      console.error('AdSense debug helper error', e);
    }
  }, 4000);
});